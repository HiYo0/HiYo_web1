...
create table member(
    ...
    mphone char(13),		
		-- char고정길이( 필드의 저장할 문자의길이가 절대적으로 고정일때 )
		-- varchar가변길이( 필드의 저장할 문자의 길이가 며로학하지 않을때 )
	mname varchar(10),		-- 회원명 [ 최대 10글자 ]
    confirm boolean, 		-- 광고문자 전송여부 boolean true/false 1/0
    joindate datetime		-- db(대소문자구별x) , 날짜 0000-00-00 0:00:00
    -- 마지막 필드 뒤에 , (쉼표) 제거
);
	# create table 테이블명( 필드명 속성 )
...
# 1. 레코드( = 필드값 ) 삽입 ( = insert into member values(값1 , 값2 , 값3, 값4, 값5 , 값6 ) )
insert into member values(1,'qwe','010-0000-0000','유재석',false,'2024-01-22 16:07:22');
	# 리터널 값 넣을때는 숫자/논리 , 문자'' , 날짜/시간 ''
...
insert into member2(회원번호1) values( 1 );		-- ( 특정 필드에 값 대입시 필드명 명시/작성 ) 오휴 : 회원번호2필드가 not null 이기때문에
insert into member2(회원번호2) values( 2 );		-- 회원번호1 필드에 null자동대입, 회원번호2필드에 2대입
	# 테이블 ( 모든 레코드 ) 확인
select *from member2;
...
insert into member3 value();		# 레코드 삽ㅇ비시 필드값이 없을경우 기본값으로 대입
insert into member3 value(10,20,'일본',false,"2024-01-22 16:44:00");
select * from member3;
...
# 계층구조 : 회원(상위) 게시판(하위)
# 테이블 1개 : 서로다른 목적의 데이터들을 하나의 테이블로 작성시 이상현상( 중복/불필요한정보 )
# 테이블 2개(정규화) : 동일한 목적의 데이터끼리 테이블을 생성해서 연결/관계(pk-fk) 이용한 관계형 데이터베이스 구측
...
create table member5(
    ...
    mname varchar(20) default "익명",		-- insert 할떄 필드의 값이 있을경우 해당값 저장 / 없으면 기본값 "익명"
    primary key(mno)  						-- mno 필드를 기본키(pk)로 사용 / 관례적 테이블 1개당 pk필드 1개 이상
);
# 5-3 게시판 테이블		# 1.게시물번호 / 2.게시물제목 / 3.게시물내용
...
create table board(
    ...
    foreign key(mno_kf)references member5(mno)

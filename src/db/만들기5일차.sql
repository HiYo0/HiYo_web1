/*
	SQL(Structured Query Language) : 데이터베이스에서 사용하는 데이터 처리언어
		1. DDL( Data Definition Languge ) 정의어		되돌리기(rollback) 불가능
			1. create	: 데이터베이스/테이블 생성
				1.데이터베이스 생성
					create database 데이터베이스명;
				2. 테이블 생성
					create table table1( 필드명 타입 제약조건 , 필드명 타입 제약조건 , 관계제약조건 );
        ...
        2. DML( Data Manipulation Language ) 조작어*
			1. insert	: 레코드 삽입
                ...
                2. 특정레코드 삽입
					- insert into 테이블먕( 필드명1 , 필드명2 )values( 값1 , 값2 );
            2. select	: 레코드 검색
				1. 모든 필드(*)를 표시한 레코드 검색
					- select * from 테이블명
                2. 특정 필드를 표시한 레코드 검색
					- select 필드명1 , select 필드명2 from 테이블명;
